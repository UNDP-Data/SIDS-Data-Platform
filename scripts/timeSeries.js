...
//data to setup colors and template
d3.json(
  "https://sids-dashboard.github.io/api/data/indicatorData-health.json"
).then((indiData) => {
  //console.log(indiData)
  const dataset = parse(indiData);
...
timeColor = d3
  ...
  const optionSelected = {
    countryGroupOption: "All",
    datasetOption: Object.keys(dataset)[0],
  };
  ...
  timeSeriesContainer
    ...
    .call((select) =>
      select
        .selectAll("option")
        .data(["All", ...Object.keys(countryGroupJson)])
        .join("option")
        .attr("value", (d) => d)
        .text((d) => d)
    )
    .on("change", () => {
     // optionSelected.countryGroupOption = d3.event.target.value;
      tempTimeChartSelection= d3.event.target.value;
...
var tempTimeChartSelection="All"

//
function updateTimeChart({ dataset, optionSelected }) {
...
//  console.log(timeData)
  ...
  function lineMark(g) {
    ...
    const pathEnter = path
      ...
      .attr("fill", "none")
      .attr("stroke", (d) => timeColor(d.country))
    ...
    path
      ...
      .attrTween("d", function (d) {
        ...
        return d3.interpolatePath(prev, next);
      })
  ...
  function lineMouseEnter(dd) {
    ...
    gMark
      ...
      .attr("fill", timeColor(dd.country))
  ...
  function mouseMoveHandler() {
    ...
    function highlightCol() {
      ...
      gMark
        ...
        .attr("fill", timeColor(closestPoint.country))
        .attr("visibility", "visible");
  ...
  function legend(g) {
    ...
    g.selectAll("text")
      ...
      .attr("fill", (d) => timeColor(d.country))
  ...
  function legendLayout(rawData) {
    ...
    if (dat[dat.length - 1].y > innerHeight) {
      ...
      const chosen = dat.filter((d, i) => d.dy > minLineHeight);
